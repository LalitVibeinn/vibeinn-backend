...
//        // ✅ Call updateVibeScore inside create function
// const points = postCategory === "general" ? 3 : 5;
// await updateVibeScore(userId, points);
// console.log(`📢 VibeScore update triggered for ${user.username} with ${points} points`);
...
//         // ✅ Create the post with an empty `likes` array
//         const post = await Post.create({
//             media: media_urls,
//             caption: fields.caption ? String(fields.caption) : "",
//             author: username,
...
//             // ✅ Send notification to the post author (if they are not the one liking it)
//             if (post.user && post.user.userId !== userId) {
//                 console.log("🔔 Sending Like Notification:", {
//                     type: "like",
//                     senderId: userId,
//                     receiverId: post.user.userId,
//                     postId: post.id,
//                     message: `${newLike.displayAuthor} liked your post`
//                 });
...
//                     post.user.userId,
//                     post.id,
//                     `${newLike.displayAuthor} liked your post`
//                 );
//             }
...
//         const userId = decodedToken.userId; // ✅ Get userId properly
//         const username = decodedToken.username;

//         const user = await User.findOne({ where: { userId } }); // ✅ Find user by userId
//         if (!user) {
//             return response.status(404).json({ message: "User not found" });
//         }
...
//             console.log(`📢 VibeScore update triggered for ${user.username} (like)`);
...
//         await Post.update({ comments }, { where: { id: post.id } });

//         console.log(`📝 Comment added by ${username}. Calling updateVibeScore...`);
...
//         // ✅ Handle Daily Comment Count and VibeScore Logic
//         const today = new Date().toISOString().split("T")[0];

//         // ✅ Reset count if it's a new day
//         if (user.lastCommentUpdate !== today) {
//             user.dailyCommentCount = 0;
//             user.lastCommentUpdate = today;
//         }

//         // ✅ Increment daily comment count
//         user.dailyCommentCount += 1;
//         console.log(`📝 ${user.username} has commented ${user.dailyCommentCount} times today.`);

//         // ✅ If 5 comments reached, increase VibeScore
//         if (user.dailyCommentCount >= 5) {
//             console.log(`🔥 ${user.username} reached 5 comments. Awarding 1 VibeScore.`);

//             // ✅ Check if the user has remaining daily VibeScore limit
//             let remainingDailyLimit = user.dailyVibePoints.remainingLimit || 0;

//             if (remainingDailyLimit > 0) {
//                 user.vibeScore += 1; // Increase VibeScore
//                 user.rank = calculateUserRank(user.vibeScore);

//                 // ✅ Deduct from daily limit
//                 user.dailyVibePoints.remainingLimit -= 1;
//                 console.log(`🎉 ${user.username} earned 1 VibeScore. Remaining limit: ${user.dailyVibePoints.remainingLimit}`);
//             } else {
//                 console.log(`⚠️ ${user.username} has reached the daily limit. VibeScore not increased.`);
//             }
...
//         // ✅ Ensure `dailyVibePoints` is an object
//         if (!user.dailyVibePoints || typeof user.dailyVibePoints !== "object") {
//             user.dailyVibePoints = { remainingLimit: 50, comment: 0 };
//         }

//         // ✅ Check if user has reached 5 comments
//         if (user.dailyCommentCount >= 5) {
//             console.log(`🔥 ${user.username} reached 5 comments. Awarding 1 VibeScore.`);
...
//         // ✅ Explicitly update `dailyVibePoints.comment` count in DB
//         user.dailyVibePoints.comment = user.dailyCommentCount;
//         user.set("dailyVibePoints", user.dailyVibePoints);
//         await user.save();
...
//                 'visibility', 'post_category', 'views', 'createdAt', 'updatedAt'
//             ]
//         });
...
//         // ✅ Map liked users to return correct display names
//         const likedBy = likedUsers.map(user => ({
//             userId: user.userId,
//             username: user.isAnonymous ? user.anonymousName || "Anonymous" : user.username,
//             profilePic: user.isAnonymous ? user.anonymousProfile : user.profile, // ✅ Show anonymous profile if user is anonymous
//         }));

//         // ✅ Process comments to include anonymous profile
//         const processedComments = await Promise.all(post.comments.map(async (comment) => {
//             const commentAuthor = await User.findOne({
//                 where: { username: comment.realUsername },
//                 attributes: ['isAnonymous', 'anonymousName', 'profile', 'anonymousProfile']
//             });

//             return {
//                 ...comment,
//                 username: commentAuthor?.isAnonymous ? commentAuthor?.anonymousName || "Anonymous" : comment.realUsername,
//                 profilePic: commentAuthor?.isAnonymous ? commentAuthor?.anonymousProfile : commentAuthor?.profile,
//                 replies: await Promise.all(comment.replies.map(async (reply) => {
//                     const replyAuthor = await User.findOne({
//                         where: { username: reply.realUsername },
//                         attributes: ['isAnonymous', 'anonymousName', 'profile', 'anonymousProfile']
//                     });

//                     return {
//                         ...reply,
//                         username: replyAuthor?.isAnonymous ? replyAuthor?.anonymousName || "Anonymous" : reply.realUsername,
//                         profilePic: replyAuthor?.isAnonymous ? replyAuthor?.anonymousProfile : replyAuthor?.profile,
//                     };
//                 }))
//             };
...
//                 author: post.author,
//                 displayAuthor: postAuthor.isAnonymous ? (postAuthor.anonymousName || "Anonymous") : postAuthor.username,
//                 profilePic: postAuthor.isAnonymous ? postAuthor.anonymousProfile : postAuthor.profile, // ✅ Show anonymous profile if anonymous
//                 visibility: post.visibility,
//                 post_category: post.post_category, // ✅ Return category
//                 totalLikes: post.likes.length,
//                 likedBy: likedBy, // ✅ Return correct liked users
//                 comments: processedComments,
//                 views: post.views,
//                 createdAt: post.createdAt,
//                 updatedAt: post.updatedAt
